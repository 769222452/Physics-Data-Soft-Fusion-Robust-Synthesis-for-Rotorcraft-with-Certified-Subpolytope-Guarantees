...
class SynthesisParams:
    ...
    # P0-1 (round-5): Gaussian std of the sampled-state residual.
    sigma_E_x: float = 5e-4
    # P0-1 (round-6): hard L2-norm cap supporting a DETERMINISTIC Loewner W_E.
    nu_E_max: float = 3e-3
...
# =========================================================
# P1-2: Single source of truth for the 8 geometric pairs shown in the paper's
# "Exp1 Geometry" figure. Defined here for cross-script consistency, but this
# script (keee-fusion.py) intentionally generates the full 10-choose-2 = 45 pairs
# for diagnostic use. The 8-pair subset rendered in the paper is FIG4_PAIRS.
# =========================================================
...
def simulate_batch_data(
    ...
    for k in range(L):
        ...
        # P0.3 + P0-1 (round-6): hard-saturated residual on 12-state block (deterministic W_E).
...
def verify_data_residual_bound(
    ...
    """P0-2 (round-6): scalar diagnostic; see monclo-fusion-comp.py for full doc.

    NOTE (keee-fusion.py): this script generates Figure 4 only and does NOT run the
    full score-processing / synthesis pipeline. The diagnostic here therefore scans ALL
    prior vertices by default and is reported as a MODEL-MISMATCH diagnostic, not as a
    residual check."""
    ...
    if verbose:
        ...
        print(f"    d_max^2 * alpha_c = {cmp_dist:.4e} | nu_E_max^2 = {cmp_nu2:.4e} (formal W_E) | "
              f"sigma_E_x^2 = {cmp_sig2:.4e}")
    return info
...
def build_structured_residual_bound(
    ...
    W_E_z_block = np.zeros((16, 16)) if W_E_z is None else np.asarray(W_E_z, dtype=float)
...
def build_psi_data(batch: Dict[str, np.ndarray], syn: SynthesisParams, d_max_override: Optional[float] = None) -> np.ndarray:
    ...
    Rtilde = 2.0 * Dt_block + 2.0 * W_E  # P0-1 paper eq:Rtilde_def
...
def simulate_tracking_with_disturbance_profile(
    ...
    for k in range(steps):
        ...
        if meas_noise_std > 0:
            # P0.3 + P0-1 (round-6): hard-saturated residual on 12-state block.
            nu = rng.standard_normal(12) * meas_noise_std
            nu = _saturate_residual_norm(nu, float(syn.nu_E_max))
            x[0:12, k + 1] += nu
...
def synthesize_controllers_for_stage3(
        syn: SynthesisParams,
        bounds: ParamBounds,
        max_vertices_proposed: int = 1024,
        score_mode: str = "lambda_max",
) -> Dict[str, object]:
    ...
    # --- P0-1: structured sampled-state residual upper bound W_E ---
    ...
    param_keys = ["sigma_t", "Jx", "Jy", "Jz", "kx", "ky", "kz", "kp", "kq", "kr"]
    plot_pairs = list(itertools.combinations(param_keys, 2))
    ...
    print(f"\n[Done] All {len(plot_pairs)} fusion heatmap plots generated in: {syn.fig_out_dir}/")
...
def main_fusion_heatmaps(
    ...
    print(f"\n{'='*60}")
    print(f"  All 45 fusion heatmap plots saved to: {syn.fig_out_dir}/")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    my_syn = SynthesisParams(
        ...
        Ru_lqr=(5, 10, 10, 10),
        fig_context="paper",
        fig_out_dir=result_path("2D")
    )

    main_fusion_heatmaps(
        syn=my_syn,
        bounds=my_bounds,
