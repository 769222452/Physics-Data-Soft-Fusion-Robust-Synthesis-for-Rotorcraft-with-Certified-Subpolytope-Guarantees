...
class SynthesisParams:
    ...
    # P0-1 (round-6): hard L2-norm cap on per-step residual ||nu(k)||_2 -> makes W_E
    # a DETERMINISTIC Loewner upper bound. Default 3e-3 ~ 6*sigma_E_x.
    nu_E_max: float = 3e-3
...
# longer generated by the default manuscript pipeline.
# =========================================================
...
def simulate_batch_data(
    ...
    for k in range(L):
        ...
        # P0-1 (round-6): hard-saturate ||nu||_2 to syn.nu_E_max so W_E is DETERMINISTIC.
...
def build_disturbance_psd_bound(
    ...
    """
    P0-3: PSD upper bound W_c on the disturbance-injection Gram matrix that dominates
    S_{c,i} S_{c,i}.T at every prior vertex (Loewner). Construction:
        alpha = safety_factor * max_i lambda_max(S_{c,i} S_{c,i}.T) over the 1024 vertices
        W_c   = alpha * I_16
...
def build_psi_data(batch: Dict[str, np.ndarray], syn: SynthesisParams, d_max_override: Optional[float] = None) -> np.ndarray:
    ...
    # P0-1: structured W_E from caller or rebuild on the fly using syn.sigma_E_x.
...
def compute_vertex_score_scalar(
    ...
    if mode == "lambda_max":  # P2.15: the canonical, paper-aligned definition (s_i^raw = lambda_max).
        return float(ev[-1])
    elif mode == "trace":
...
def synthesize_controllers_for_stage3(
    ...
    if I0_vertices:
        ...
        print(f"  [P0-2 I0] processed score-zero subset I0 is EMPTY; skipping residual diagnostic on I0")
